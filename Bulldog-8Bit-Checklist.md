...
- [x] 📄 **Hurt frame + damage blink** (`CHAR-3`): a squashed hurt pose plays for the whole ~500 ms hit beat — on a level restart and on the last heart alike — replacing the placeholder flat red tint, with a blink that alternates between damage red and the hero's **own** selected color so the hit reads even on the red bulldog. Spec `specs/hurt-frame.md`; `ANIMATIONS.hurt` (frame 33), hurt-beats-everything priority in `getAnimationKey`, and `getHurtTint` unit-tested in `src/physics/animation.js`. The source art has no damage pose, so the frame is **derived** from idle frame 0 by `tools/make-hurt-frame.py` — a dev-only, idempotent Pillow script (not in `package.json`, the build, or CI) whose output is committed as pixels: the sheet gains a 4th row, leaving every existing frame index untouched. **Playtest fix landed with it:** the selected hero color now survives a level restart (carried in the registry like hearts — it had been reset to white by `create()` since `CHAR-4`). Merged to `main` (PR #12).
- [x] 📄 **Double jump** (`MOVE-7`): allow exactly one extra mid-air jump (blocked again until landing). Spec `specs/double-jump.md`; `airJumpsUsed` counter in `src/physics/player.js`, jump-count rule unit-tested; ledge walk-off = one air jump (no coyote time). Merged to `main` (PR #4).
...
- [x] 📄 **Start screen + nickname entry** (`UI-1` minimal + `UI-2`): the game's front door — type a nickname (**required, 3-10 characters, A-Z 0-9**, drawn black at 8px so it reads as the field being edited) and pick a color with the arrows on a live tinted bulldog preview (driving the existing `CHAR-4` module, whose temporary `C` dev key is now gone), then ENTER to start. Spec `specs/start-screen.md`; pure rules unit-tested in `src/state/nickname.js` plus `previousColor()` added to `src/state/color-select.js`; the scene itself is `src/scenes/start-scene.js`, the first file in the `src/scenes/` directory. **The start screen now owns "a new run"** — it resets hearts and score into the registry, and `STATE-3`'s GAME OVER overlay returns here instead of restarting the level. The game's title is **Bulldog Rush** (shown as `BULLDOG RUSH`). *(Two long-standing bugs fixed alongside: the registry score mirror kept its pre-hit total after a level restart, and the fullscreen button kept DOM focus so every SPACE jump re-toggled fullscreen.)*
- [x] 📄 **HUD + the game's pixel font** (`UI-3` Alpha subset): hearts top-left, a zero-padded `SCORE 000400` top-right (no Energy, no timer), **and all text switched to Press Start 2P** — vendored with its SIL OFL licence and loaded via the browser's own `FontFace` API (no npm package, no CDN). Spec `specs/hud-and-font.md`; shared styles in `src/ui/text-style.js`, padding unit-tested in `src/state/score.js`. The game boots behind a font gate so the first frame is already in the right font, and falls back to monospace if the font can't load.
