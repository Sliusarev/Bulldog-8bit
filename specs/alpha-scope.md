...
> enemy, 3 hearts, nickname, score, results window, double jump) stays inside one
...
and the final score. Everything else is Beta.
...
- **Small Bones** (`SCORE-1`): collectible, **+100 points** each.
- **One arcade score** (`SCORE-8`): a single points total — bone +100, stomp +200.
  *(The level timer, `SCORE-7`, was moved to Beta: two metrics made the
  player's result ambiguous and a scoreboard unsortable.)*
...
- **HUD** (`UI-3` subset): hearts and the score (no Energy, no timer).
...
- [ ] The score rises by 100 per bone and 200 per stomped enemy, and is visible in the HUD.
...
- [ ] The results window shows the nickname and the final score.
...
- HUD (top of screen): hearts (3) and the score.
...
## 6. Data & persistence
- Run-scoped state only (nickname, color, score, hearts) held in
...
> **Note on level restart + score:** resolved — the score **resets to 0** with
> the level (the bones respawn, so keeping it would double-count them) and the
> hearts carry over. See `specs/health-hearts.md`. *(Originally this also asked
> what a timer should do on restart; the timer is no longer in Alpha.)* Resolve
